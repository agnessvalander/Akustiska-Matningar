function readMWLdaq_dir()
%readMWLdaq_dir OBTAINS THE SPECTRA FROM ALL .dat FILES IN THE CHOSEN
%DIRECTORY, AND SAVES THEM IN A SUB-DIRECTORY "mat-files"
% ----------------------------------------------------------------------- %
strflag = 'spectra';
files_dir = uigetdir('.','Pick a Directory');
output_dir = fullfile(files_dir,'mat-files');
mkdir(output_dir);
allFiles = dir(fullfile(files_dir,'*.dat'));
numFiles = length(allFiles);
f = waitbar(0,'Extracting spectra from all .dat-files in the directory...');
for ii = 1:numFiles
   file_ii = fullfile(allFiles(ii).folder,allFiles(ii).name);
   [~,spectra] = readMWLdaq_LV(file_ii,strflag);
   save(fullfile(output_dir,[allFiles(ii).name(1:end-4) '.mat']),'spectra')
   waitbar(ii/numFiles,f)
end
waitbar(1,f,'Done!')
pause(0.5)
close(f)
...
if strcmp(strflag,'all') || strcmp(strflag,'spectra')
 ...
 while(~strcmp(lstr,'EndOfConfiguration_HubbaBubba') && ~feof(fid))
  lstr=fgetl(fid);
 end
 spectra.analysis=fgetl(fid);
